	...
	co.plugin.plugins = {
		...
		jquery_ui_slider : {
			...
			css : [ "resource/plugins/jquery/ui/slider/jquery-ui.min.css", "resource/plugins/jquery/ui/slider/jquery-ui.theme.min.css",
					"resource/plugins/jquery/ui/slider/jquery-ui-slider-pips.min.css" ]
		},
		jquery_sortable : {
			js : [ "resource/plugins/jquery/ui/interactions/jquery-ui.min.js" ],
			css : [ "resource/plugins/jquery/ui/interactions/jquery-ui.min.css", "resource/plugins/jquery/ui/widgets/jquery-ui.theme.min.css" ]
...
(function() {
	co.input.recallSliderPips = function() {
		/*
		 * ! jQuery-ui-Slider-Pips - v1.7.2 - 2014-10-19 Copyright (c) 2014 Simon Goellner
		 * <simey.me@gmail.com>; Licensed MIT
		 */
		...
		(function($) {
			...
			var extensionMethods = {
				...
				pips : function(settings) {
					...
					var slider = this, collection = "", pips = (slider.options.max - slider.options.min) / slider.options.step, options = {
					...
					function labelClick(label) {
						...
						if (true === slider.options.range) {
							...
							if (sliderVals[0] === sliderVals[1]) {
								...
								// ...or if the first handle is closest to our label, bring that
								// handle.
							} else {
					...
					function createPip(which) {
						...
						return "<span class=\"" + classes + "\" style=\"" + css + "\">" + "<span class=\"ui-slider-line\"></span>" + "<span class=\"ui-slider-label\" data-value=\"" + labelValue
								+ "\">" + options.formatLabel(label) + "</span>" + "</span>";
					...
					if (slider.options.values) {
						...
						newClass = [ "ui-slider-pip-" + slider.options.values[0] + " ui-slider-pip-selected-initial-first",
				...
				float : function(settings) {
					...
					if (options.handle) {
						...
						if (slider.options.values) {
							...
							$tip = [ $("<span class=\"ui-slider-tip\">" + options.formatLabel(vals[0]) + "</span>"), $("<span class=\"ui-slider-tip\">" + options.formatLabel(vals[1]) + "</span>") ];
					...
					if (options.pips) {
						...
						slider.element.find(".ui-slider-label").each(function(k, v) {
							...
							var $this = $(v), val = $this.data("value"), label = $this.data("value"), $tip;
